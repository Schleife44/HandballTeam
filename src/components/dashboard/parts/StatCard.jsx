...
const StatCard = ({ label, value, trend, icon: Icon, invertTrendColor }) => {
  const isPositiveTrend = invertTrendColor ? trend < 0 : trend > 0;
    ...
    <div className="bg-zinc-900/30 backdrop-blur-md border border-zinc-800/40 rounded-2xl lg:rounded-[2rem] p-4 lg:p-6 hover:border-brand/30 transition-all group">
      <div className="flex items-center justify-between mb-4">
        ...
        {trend !== undefined && trend !== null && trend !== 0 && (
          <div 
            title="vs. Saison-Durchschnitt (letzte 3 Spiele)"
            className={`flex items-center gap-1 px-2 py-1 rounded-full text-[9px] font-black ${isPositiveTrend ? 'bg-brand/10 text-brand' : 'bg-red-500/10 text-red-500'}`}
          >
